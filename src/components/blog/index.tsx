...
const Blog: Component<{ props: BlogMetadata; children: JSX.Element }> = ({
  props,
  children,
}) => {
  let link = props.link
  ...
  return (
    <div class="flex justify-center w-full pt-20 bg-gradient-to-b from-[#202124] to-gray-800">
      <div class="w-full max-w-5xl mb-10 mx-3">
        ...
        <Meta property="og:author" content={props.authors.join(', ')} />
        <Meta property="og:image" content={props.imageUrl} />
        <Meta
          property="article:published_time"
          content={new Date(props.publishedDate).toISOString()}
        />
        <div class="mb-3">
          <p class="text-white font-bold text-xl px-2">
            <a href="/" class="hover:text-[#edc567]">
              burn
            </a>
            ...
            <For
              ...
              children={(path) => (
                <span>
                  <span>{' · '}</span>
                  <a href={path.href} class="hover:text-[#edc567]">
                    {path.name}
                  </a>
                </span>
              )}
        ...
        <article class="blog rounded-lg bg-white/5 pt-4">
          <div>
            <h1 class="px-3 sm:px-8">{props.title}</h1>
